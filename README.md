...
| `forge-build-from-zero-skill.zip` | **From an empty workspace to a complex installed app with only a PAT** — the Forge driven with the `esoul` Python SDK (`pip install esoul` ≥ 0.17: `esoul.forge` and the `esoul-forge` CLI — sync, drive, look, check, issues and resolve as data; raw HTTP as the fallback; no repo, no MCP), the full esoul-sdk surface (the SDK's documentation is bundled verbatim), and the CLOSED DEBUGGING LOOP: the box's problem journal that leads every tool answer, the board's Issues tab, `resolve_problem`, parking on issues, VIEW AS personas, headless drives — plus every lesson from building Stream Lab and the shop with the owner (access design, machines and bridges, charts, UI craft, what wedges a box). Read before the two Forge skills above when the session has a PAT. |
