...
| `forge-realtime-apps-skill.zip` | Build an ExternalSoul app whose **background tasks update the UI live** (durable Inngest tasks publishing on the app's realtime channel) and that brings its **own server functions** (ops for server truth, streaming routes) — with `esoul-sdk` in the Forge workbench over MCP; drive tasks/ops/routes/realtime before install; push to the app's own GitHub repo; install from a link in Settings → Apps. The companion to `forge-app-builder` for apps with a backend. |
